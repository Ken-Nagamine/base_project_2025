...
import EditBrand from "../edit-brand-dialog";
import { 
  AlertDialog, 
  AlertDialogAction, 
  AlertDialogCancel, 
...
import ShowBrandDialog from "../show-brand-dialog";
...
export const columns: ColumnDef<Brand>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => {
          const imageUrl = row.getValue<string | null>("logo_url");
    
          return (
            // <Avatar className="h-10 w-10">
            //   <AvatarImage src={imageUrl || undefined} alt="Imagem categoria" />
            // </Avatar>
            <img src={imageUrl || undefined} alt="Brand logo" className="h-12"/>
          );
      },
    },
    {
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        <div className="flex justify-center gap-3">
            ...
            <AlertDialog>
              ...
              <AlertDialogContent>
                <AlertDialogHeader>
                  ...
                  <AlertDialogDescription>
                    Esta ação não pode ser desfeita. Isso irá deletar permanentemente a Brand{' '}
                    **"{brand.name}"** do banco de dados.
                  </AlertDialogDescription>
                </AlertDialogHeader>
                <AlertDialogFooter>
                  ...
                  <AlertDialogAction onClick={() => {
                    router.delete(route('admin.brands.destroy', Number(id)), {
                            onSuccess: () => {
                                // Opcional: mostrar uma notificação de sucesso
                                console.log('Brand excluído com sucesso!');
                            },
                            onError: (errors) => {
                                console.error('Erro ao excluir brand:', errors);
                            }
                        });
            ...
            <ShowBrandDialog brand={row.original}/>
        </div>
        )
      },
    },
